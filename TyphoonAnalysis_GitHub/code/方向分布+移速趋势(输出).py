            ...
            if line.startswith('66666'): 
                ...
                if len(parts) >= 8: # 确保行中包含足够的组成部分
                    ...
                    if 80 <= full_year <= 99: # 1980-1999 年的台风
                        ...
                        print(f"Warning: Unrecognized year prefix {typhoon_year_prefix} in typhoon ID {typhoon_id_code_str} from file {filepath}. Using XXXX as year.")
                        full_year_str = "XXXX" 
                    
                    current_typhoon_sid = f"{full_year_str}{typhoon_num_suffix.zfill(2)}"
                    current_typhoon_name = parts[7] # 例如 'Sarah', 'EWINIAR'
                else:
                    print(f"Warning: Header line in {filepath} has fewer than 8 parts: '{line}'. Skipping header.")
                    current_typhoon_sid = None
                    current_typhoon_name = None
                    continue
            ...
            # Example data line: 1983062300 0 140 1170 1004 10
                ...
                if len(parts) >= 6:
                    try:
                        ...
                        iso_time = pd.to_datetime(f"{year}-{month:02d}-{day:02d} {hour:02d}:00:00")
                        ...
                        data.append([current_typhoon_sid, current_typhoon_name, iso_time, type_code, lat, lon, pressure, wind_ms])
                    except (ValueError, IndexError) as e:
                        print(f"解析轨迹数据行失败 (台风ID: {current_typhoon_sid}, 行: '{line}'), 错误: {e}")
                        continue
                else:
                    print(f"Warning: Track data line for {current_typhoon_sid} has fewer than 6 parts: '{line}'. Skipping.")
    
    if not data:
        print(f"Warning: No valid data parsed from {filepath}.")
        return pd.DataFrame()

    df_parsed = pd.DataFrame(data, columns=['SID', 'NAME', 'ISO_TIME', 'TYPE_CODE', 'LAT', 'LON', 'PRESSURE', 'WIND_MS'])
...
if not os.path.exists(cma_data_dir):
    print(f"ERROR: Data directory '{cma_data_dir}' does not exist. Please ensure this directory is created and CMA data files are placed inside it.")
...
if not df_region.empty:
    ...
    for sid in df_region['SID'].unique():
        typhoon_track = df_region[df_region['SID'] == sid].sort_values(by='ISO_TIME').reset_index(drop=True)
        ...
        for i in range(1, len(typhoon_track)): # Start from the second point to compare with previous
            ...
            is_over_land = (current_point['LAT'] >= min_land_lat) and \
                           (current_point['LAT'] <= max_land_lat) and \
                           (current_point['LON'] >= min_land_lon) and \
                           (current_point['LON'] <= max_land_lon)
            
            is_strong_enough = (current_point['TYPE_CODE'] >= min_landfall_type_code) and \
                               (current_point['WIND_KMH'] >= min_landfall_wind_kmh)

            # Check if previous point was NOT over "land" (i.e., over sea)
            was_over_sea_prev = not ((previous_point['LAT'] >= min_land_lat) and \
                                     (previous_point['LAT'] <= max_land_lat) and \
                                     (previous_point['LON'] >= min_land_lon) and \
                                     (previous_point['LON'] <= max_land_lon))
    ...
    def get_pre_landfall_data_schemeB(sid, landfall_time, df_full_track):
        track = df_full_track[df_full_track['SID'] == sid].sort_values(by='ISO_TIME').reset_index(drop=True)
        ...
        if len(data_24h_pre_landfall) < 2: 
            print(f"Warning: Typhoon {sid} has insufficient data points ({len(data_24h_pre_landfall)}) within 24 hours before landfall. Skipping.")
            return None
...
if not df_pre_landfall.empty:
    ...
    df_pre_landfall['time_diff_hours'] = (df_pre_landfall['next_time'] - df_pre_landfall['ISO_TIME']).dt.total_seconds() / 3600

    # Calculate speed (km/h)
    df_pre_landfall['speed_kmh'] = df_pre_landfall['distance_km'] / df_pre_landfall['time_diff_hours']
    
    # NEW (新增): Calculate bearing (方位角) and categorize direction (方向类别)
    ...
    for sid in df_pre_landfall['SID'].unique():
        ...
        # Determine the most frequent direction for the 24h pre-landfall period
        # 对比 (Contrast): 移速是平均值 (Mean)，方向是众数 (Mode) 或最频繁的类别 (Most Frequent Category)。
        dominant_direction = None
        if not valid_directions.empty:
            direction_counts = valid_directions.value_counts()
            if not direction_counts.empty:
                dominant_direction = direction_counts.idxmax() # 取出现次数最多的方向
...
if not df_avg_data_per_typhoon.empty:
    ...
    sns.regplot(x='YEAR', y='Avg_Speed_24h_kmh', data=df_analysis_speed, 
                scatter_kws={'alpha':0.6, 'color':'blue'}, 
                line_kws={'color':'red', 'linestyle':'--'},
                ci=95) # Add confidence interval (添加 95% 置信区间)

    # 大标题 (Main Title): 务必清晰、信息量大
...
# --- Step 5.1 (New): Visualization of Movement Direction Distribution ---
...
if not df_avg_data_per_typhoon.empty:
    ...
    if not df_analysis_direction.empty:
        # Define the order of categories for plotting (确保方向顺序正确)
        ...
        sns.countplot(x='Dominant_Direction_24h', data=df_analysis_direction, palette='viridis', order=direction_order)
        ...
        # Etymology (词源): "Distribution" 源于拉丁语 *distributio*，意为“分配，散布”。
...
if not df_avg_data_per_typhoon.empty and len(df_avg_data_per_typhoon.dropna(subset=['Avg_Speed_24h_kmh'])) >= 2:
    df_analysis_speed = df_avg_data_per_typhoon.dropna(subset=['Avg_Speed_24h_kmh']).copy()
    slope, intercept, r_value, p_value, std_err = linregress(df_analysis_speed['YEAR'], df_analysis_speed['Avg_Speed_24h_kmh'])

    print(f"Linear Regression Results (Movement Speed):")
...
if not df_avg_data_per_typhoon.empty and len(df_avg_data_per_typhoon.dropna(subset=['Avg_Speed_24h_kmh'])) >= 2:
    if p_value < 0.05:
        ...
        print(f"    结果显示，在登陆前24小时的平均移动速度随年份变化呈现显著的{trend_direction_speed}趋势（线性回归p值 = {p_value:.3f}，R² = {r_value**2:.3f}），平均速度以每年 {abs(slope):.3f} km/h/年的速度{trend_direction_speed}。这一发现与H1（速度）中提出的台风在登陆前减慢（Stalling）的假设 [如果slope < 0 则为一致，否则为不一致] 。")
    else:
        print(f"    统计分析（线性回归p值 = {p_value:.3f}）表明，台风登陆前平均速度随年份变化的趋势不具有统计显著性。这意味着在1980-2024年间，中国华南/华东登陆台风在登陆前24小时的平均移动速度没有观测到显著的长期变化。")
else:
    print("    由于速度数据不足，无法得出关于速度趋势的统计结论。")
print(f"    图1（即你Step 5生成的速度趋势图像）展示了各年度登陆台风的平均速度及趋势线。")

# Direction results (方向结果)
if not df_analysis_direction.empty:
    most_common_direction = df_analysis_direction['Dominant_Direction_24h'].mode()
    print(f"    在台风移动方向方面，登陆前24小时主导方向频次分布显示，最常见的移动方向是 {most_common_direction} 方向。这表明在该区域，台风在登陆前倾向于沿此方向移动。")
    print(f"    图2（即你Step 5.1生成的方向分布图像）展示了主导移动方向的频次分布。")
else:
...
print("2. **趋势原因 (移速，如果显著)**: 探讨观测到的速度变化（减慢/加快）可能的气候学原因。例如，与西太平洋副热带高压的变化、大尺度环流模式的调整、海洋热力学条件变化等是否有联系？")
print("3. **趋势原因 (移速，如果不显著)**: 探讨为什么没有观测到显著移速趋势。是区域性差异？是数据本身的局限性（例如CMA数据在早期年份的精度、6小时采样间隔的局限性）？还是有其他相互抵消的因素？")
print("4. **移动方向特征**: 分析主导移动方向的分布，这与你对该地区台风路径的现有了解是否一致？是否存在主导方向随年份变化的趋势？（如果进一步分析的话）")
print("5. **移速与方向的关联**: 台风的移速慢是否与某个特定移动方向相关联？例如，在某些方向上，台风是否更容易“Stall”？这可能与区域性的天气系统或地形作用有关。")
print("6. **方法论局限性**: 本研究中，通过边界框近似识别登陆点可能带来的不确定性。是否有可能存在“擦边而过”或在非大陆区域（如台湾岛、海南岛）登陆但被本方法识别为大陆登陆的情况？这会如何影响结果？")
print("7. **数据质量**: 早期CMA数据的质量和密度与近期数据相比有何不同？这可能对趋势分析产生何种影响？")
print("8. **与现有研究对比**: 你的发现与国际上关于台风移动速度变化（Typhoon Translation Speed Change）和方向变化（Typhoon Movement Direction Change）的现有研究（如Kossin et al., 2014; Hall and Kossin, 2019）是否一致？有哪些异同？如果存在差异，可能的原因是什么？")
# Contrast (对比): 移速变化与方向变化的结合分析，更能揭示台风行为的复杂性。
# Etymology (词源): "Discussion" 源于拉丁语 *discussio*，意为“检查，审查”。
print("9. **未来工作**: 进一步研究可以包括哪些方面？例如，更精细的登陆点识别（使用海岸线Shapefile）、分区域（华南/华东）分析、不同强度等级台风的对比分析、引入更多环境变量（如海表温度、垂直风切变）进行回归分析，以及对移动方向进行更精细的**子时段分析** (sub-period analysis) 或**聚类分析** (cluster analysis) 以识别常见的路径类型。")
