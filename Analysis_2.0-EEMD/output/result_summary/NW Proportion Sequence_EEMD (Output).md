```markdown
# NW占比序列：EEMD + 频谱匹配 + 显著性检验 输出报告
```

## 1. 运行环境与命令

```powershell
PS D:\下载过渡\TyphoonAnalysis\result_2.0_研究分析过程> & C:\Users\DrXen\AppData\Local\Microsoft\WindowsApps\python3.12.exe d:/下载过渡/TyphoonAnalysis/result_2.0_研究分析过程/代码/NW占比序列_EEMD+频谱匹配+显著性检验+绘图.py
```
...
*注：p 值接近 1 表示该分量不显著（可能为噪声），p=0.124 的 IMF 3 边际显著。*
