...
speed = df["Avg_Speed_24h_kmh"].values
N = len(speed)

# ============================
# 2. 执行 EEMD
# ============================
imfs = emd.sift.ensemble_sift(speed)
...
for i in range(num_imfs):
    ...
    p_values.append(p)
    print(f"IMF {i} 显著性 p = {p:.9f}")

# ============================
# 6. 绘图（含 QDO）
# ============================
plt.figure(figsize=(12,8))
plt.plot(years, speed, label="Original Speed", color="black", zorder=5)
...
plt.legend()
plt.title("EEMD 分解：台风登陆前 24h 平均移速（含 Trend / PDO / ENSO / QDO）")
plt.xlabel("Year")
plt.ylabel("Speed (km/h)")
plt.grid(True)
plt.savefig(os.path.join(FIGURE_DIR, "EEMD_speed_components_QDO.png"), dpi=300)
plt.show()
...
plt.ylabel("1 - p")
plt.legend()
plt.savefig(os.path.join(FIGURE_DIR, "IMF_significance_speed.png"), dpi=300)
plt.show()
